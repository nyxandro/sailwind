...
const SEGMENTS = 256;
const TERRAIN_ROWS_PER_TASK = 16;
const DEPTH_ROWS_PER_TASK = 64;
...
export async function createSeabed(pause) {
  ...
  try {
    ...
    for (let row = 0; row <= SEGMENTS; row++) {
      for (let col = 0; col <= SEGMENTS; col++) {
        ...
        color
          .copy(sand)
          .lerp(shelf, smooth(7, 26, sample.depth))
          .lerp(deep, smooth(24, MAX_SEABED_DEPTH, sample.depth));
      ...
      if (row % TERRAIN_ROWS_PER_TASK === 0) await pause();
    }
    ...
    const cellAt = (value) => {
      if (value < COORDINATES[0] || value > COORDINATES[SEGMENTS])
        throw new Error('SEABED_SAMPLE_OUT_OF_RANGE: Requested point is outside the seabed');
      let low = 0;
      let high = SEGMENTS;
      while (high - low > 1) {
    ...
    for (let row = 0; row < mapSize; row++) {
      ...
      if (row % DEPTH_ROWS_PER_TASK === 0) await pause();
    }
    depthMap = new THREE.DataTexture(data, mapSize, mapSize);
